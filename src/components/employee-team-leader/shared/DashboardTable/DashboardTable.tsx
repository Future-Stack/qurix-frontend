...
export function DashboardTable<T>(props: DashboardTableProps<T>) {
  ...
  if (isLoading) {
    ...
    bodyContent = data.map((item, index) => {
        ...
        >
          {columns.map((column) => {
            ...
            return (
              <td
                key={column.header}
                className={`px-6 py-4 text-[13px] font-medium text-[#0F172A] align-middle whitespace-nowrap ${alignClass}`}
              >
                {column.render
            ...
            {columns.map((column) => {
              ...
              return (
                <th
                  key={column.header}
                  scope="col"
                  className={`px-6 py-4 text-[14px] font-medium text-[#282828] tracking-wider whitespace-nowrap font-condensed ${alignClass}`}
                >
                  {column.header}
                </th>
              );
