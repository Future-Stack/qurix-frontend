...
export default function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
    ...
    const ToggleSwitch = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
        <button
            onClick={onChange}
            type="button"
            className={`w-[44px] h-[24px] rounded-full p-0.5 transition-colors cursor-pointer flex items-center shrink-0 ${checked ? 'bg-[#06530b] justify-end' : 'bg-gray-300 justify-start'
                }`}
        >
