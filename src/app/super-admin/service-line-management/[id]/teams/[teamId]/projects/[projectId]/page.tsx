...
function formatTimeForDisplay(timeStr: string): string {
  ...
  if (parts.length >= 2 && parts[0] !== undefined && parts[1] !== undefined) {
    ...
    hours = hours % 12 || 12;
    return `${hours < 10 ? '0' + hours : hours}:${parts[1]} ${ampm}`;
  }
  return timeStr;
}
...
export default function SuperAdminTeamProjectDetailsPage() {
  const router = useRouter();
  const params = useParams();
  const projectId = params?.projectId ? String(params.projectId) : 'FO2D9BC6E142';
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [saved, setSaved] = useState(false);

  // Pre-filled form state (existing project data)
  const [clientName, setClientName] = useState('alexjlouis');
  const [fiverrOrderId, setFiverrOrderId] = useState(projectId || 'FO41BDCFC4F84');
  ...
  const [deliveryDeadline, setDeliveryDeadline] = useState('2026-08-15');
  const [deliveryTime, setDeliveryTime] = useState('16:57');
  const [projectValue, setProjectValue] = useState('$3,615');

  // Pre-filled uploaded files
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([
    { id: '1', name: 'Requirement_v1.pdf', size: '2.4 MB' },
    { id: '2', name: 'Wireframes.fig', size: '15.2 MB' },
  ]);

  // Pre-filled resource links
  const [addedLinks, setAddedLinks] = useState<ResourceLink[]>([
    { id: '1', type: 'Figma', url: 'https://figma.com/design/qurix-app' },
    { id: '2', type: 'Google Drive', url: 'https://drive.google.com/drive/folders/qurix' },
  ]);

  const [activeLinkType, setActiveLinkType] = useState<'Figma' | 'Google Drive' | 'GitHub' | 'External URL' | null>(null);
  const [linkInput, setLinkInput] = useState('');

  // Team members (pre-selected)
  const [memberSearch, setMemberSearch] = useState('');
  const [members, setMembers] = useState<TeamMemberItem[]>([
    { id: '1', name: 'Kathryn Murphy', role: 'Lead Designer', handle: '@alex.chen', empId: 'EMP001', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80', selected: true },
    { id: '2', name: 'Annette Black', role: 'UI/UX Designer', handle: '@sarah.chen', empId: 'EMP002', avatar: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=150&q=80', selected: true },
    { id: '3', name: 'Courtney Henry', role: 'Backend Engineer', handle: '@mike.r', empId: 'EMP003', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80', selected: true },
    { id: '4', name: 'Robert Fox', role: 'Product Manager', handle: '@james.liu', empId: 'EMP004', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=150&q=80', selected: false },
    { id: '5', name: 'Kristin Watson', role: 'QA Lead', handle: '@emily.park', empId: 'EMP005', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=150&q=80', selected: false },
  ]);
  ...
  const removeLink = (id: string) => setAddedLinks(prev => prev.filter(l => l.id !== id));

  const toggleMember = (id: string) => setMembers(members.map(m => m.id === id ? { ...m, selected: !m.selected } : m));
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="font-['Roboto'] text-[14px] text-[#a19791] mt-0.5">
              Admin Panel · Order ID: {fiverrOrderId}
            </p>
          </div>
        </div>
      ...
      <div className="w-full max-w-[975px] mx-auto border-x border-[#DADADA] px-4 py-6 md:px-8 md:py-8 flex-1 min-h-0 overflow-y-auto no-scrollbar space-y-6">
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-[24px] p-6 space-y-6 shadow-2xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-['Roboto'] font-bold text-[18px] text-[#3c3c3c]">Project Information</h2>
              <p className="font-['Roboto'] text-[13px] text-[#a19791]">Core project details and metadata</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="space-y-1.5">
              <label className="font-['Roboto'] font-medium text-[13px] text-[#3c3c3c]">Client Name <span className="text-red-500">*</span></label>
            ...
            {/* Fiverr Order ID */}
            <div className="space-y-1.5">
              <label className="font-['Roboto'] font-medium text-[13px] text-[#3c3c3c]">Fiverr Order ID <span className="text-red-500">*</span></label>
              <div className="relative">
                <Hash className="absolute left-3.5 top-1/2 -translate-y-1/2 size-4 text-gray-400" />
                <input type="text" value={fiverrOrderId} onChange={e => setFiverrOrderId(e.target.value)}
            ...
            {/* Start Date */}
            <div className="space-y-1.5">
              <label className="font-['Roboto'] font-medium text-[13px] text-[#3c3c3c]">Start Date <span className="text-red-500">*</span></label>
              <div className="relative flex items-center">
                <input type="text" readOnly value={formatDateForDisplay(startDate)} placeholder="MM/DD/YYYY"
            ...
            {/* Delivery Deadline */}
            <div className="space-y-1.5">
              <label className="font-['Roboto'] font-medium text-[13px] text-[#3c3c3c]">Delivery Deadline <span className="text-red-500">*</span></label>
              <div className="relative flex items-center">
                <input type="text" readOnly value={formatDateForDisplay(deliveryDeadline)} placeholder="MM/DD/YYYY"
                  className="w-full bg-white border border-[#e8eaf0] rounded-[16px] pl-4 pr-11 py-3 text-sm text-[#3c3c3c] font-medium focus:outline-none focus:border-[#06530b] cursor-pointer transition-all" />
                <Calendar className="absolute right-4 size-4.5 text-[#616161] pointer-events-none stroke-[1.75]" />
            ...
            {/* Deadline Time */}
            <div className="space-y-1.5">
              <label className="font-['Roboto'] font-medium text-[13px] text-[#3c3c3c]">Deadline Time</label>
              <div className="relative flex items-center">
                <input type="text" readOnly value={formatTimeForDisplay(deliveryTime)} placeholder="hh:mm AM/PM"
                  className="w-full bg-white border border-[#e8eaf0] rounded-[16px] pl-4 pr-11 py-3 text-sm text-[#3c3c3c] font-medium focus:outline-none focus:border-[#06530b] cursor-pointer transition-all" />
            ...
            {/* Project Value */}
            <div className="space-y-1.5">
              <label className="font-['Roboto'] font-medium text-[13px] text-[#3c3c3c]">Project Value <span className="text-red-500">*</span></label>
              <div className="relative">
                <DollarSign className="absolute left-3.5 top-1/2 -translate-y-1/2 size-4 text-[#06530b]" />
                <input type="text" value={projectValue} onChange={e => setProjectValue(e.target.value)} placeholder="$1,000"
          ...
          <div className="bg-[#e6f4ea] border border-emerald-200/80 p-4 rounded-[16px] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="font-['Roboto'] font-medium text-[13px] text-[#06530b]">Remaining Timeline</span>
                <h3 className="font-['Roboto'] font-bold text-[18px] text-[#06530b] leading-tight">15D 4H 03M 07S</h3>
              </div>
            </div>
            <div className="flex flex-col items-start sm:items-end gap-1.5">
              <span className="font-['Roboto'] font-semibold text-[12px] text-[#06530b]">Timeline Health</span>
              <div className="w-48 h-2.5 bg-emerald-200/80 rounded-full overflow-hidden flex">
                <div className="h-full bg-[#06530b] w-[60%] rounded-full" />
              </div>
            </div>
          </div>
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-[24px] p-6 space-y-5 shadow-2xs">
          ...
          <div className="bg-[#fcfcfc] border border-[#e5e7eb] rounded-[16px] p-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="size-10 rounded-full bg-[#06530b] text-white font-bold flex items-center justify-center text-lg overflow-hidden shrink-0">S</div>
              <div>
                <h4 className="font-['Roboto'] font-bold text-[15px] text-[#3c3c3c]">Softvence</h4>
                <p className="font-['Roboto'] text-[12px] text-[#828282]">Platform Admin · @alex.chen · EMP001</p>
              </div>
            </div>
            <span className="bg-gray-100 border border-gray-200 text-[#828282] text-[12px] font-medium px-3 py-1 rounded-full flex items-center gap-1.5">
              <Lock className="size-3" /> <span>Cannot be changed</span>
            </span>
          </div>
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search by name, username, or Employee ID..."
          ...
          <div className="divide-y divide-[#f0f0f0]">
            {filteredMembers.map(member => (
              <div key={member.id} onClick={() => toggleMember(member.id)}
                ...
                <div className="flex items-center gap-3">
                  <img src={member.avatar} alt={member.name} className="size-10 rounded-full object-cover shrink-0 border border-gray-100" />
                  <div>
                    <h5 className="font-['Roboto'] font-bold text-[14px] text-[#3c3c3c]">{member.name}</h5>
                    <p className="font-['Roboto'] text-[12px] text-[#828282]">{member.role} · {member.handle} · {member.empId}</p>
                  </div>
                </div>
                <div className={`size-5 rounded-full border flex items-center justify-center transition-all ${member.selected ? 'bg-[#06530b] border-[#06530b] text-white' : 'border-gray-300 bg-white'}`}>
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-[24px] p-6 space-y-6 shadow-2xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-['Roboto'] font-bold text-[18px] text-[#3c3c3c]">Project Requirements</h2>
              <p className="font-['Roboto'] text-[13px] text-[#a19791]">Attach files, links, and resources for the project</p>
            </div>
          </div>
          ...
          <div onClick={() => fileInputRef.current?.click()}
            ...
            <div>
              <h4 className="font-['Roboto'] font-bold text-[15px] text-[#3c3c3c]">Drag & drop files here</h4>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
              {['PDF', 'DOC', 'XLSX', 'PPTX', 'ZIP', 'PNG', 'MP4'].map(fmt => (
                <span key={fmt} className="bg-gray-200/80 text-[#575757] text-[11px] font-semibold px-2.5 py-0.5 rounded-full uppercase">{fmt}</span>
              ))}
            </div>
          </div>
          ...
          {uploadedFiles.length > 0 && (
            <div className="space-y-2 pt-2">
              <h5 className="font-['Roboto'] font-semibold text-[13px] text-[#3c3c3c]">Attached Files ({uploadedFiles.length}):</h5>
          ...
          <div className="relative flex items-center justify-center my-4">
            ...
            <span className="absolute bg-white px-4 text-[11px] font-bold text-[#828282] uppercase tracking-wider">OR ADD LINK</span>
          </div>

          <div className="flex flex-wrap items-center justify-center gap-3">
            <button onClick={() => setActiveLinkType('Figma')}
              ...
              <FileText className="size-4 text-purple-600" /> <span>Figma +</span>
            </button>
            <button onClick={() => setActiveLinkType('Google Drive')}
              ...
              <Globe className="size-4 text-blue-600" /> <span>Google Drive +</span>
            </button>
            <button onClick={() => setActiveLinkType('GitHub')}
              ...
              <GithubIcon className="size-4 text-slate-800" /> <span>GitHub +</span>
            </button>
            <button onClick={() => setActiveLinkType('External URL')}
              ...
              <LinkIcon className="size-4 text-[#06530b]" /> <span>External URL +</span>
            </button>
          </div>
          ...
          {addedLinks.length > 0 && (
            <div className="space-y-2 pt-2">
              <h5 className="font-['Roboto'] font-semibold text-[13px] text-[#3c3c3c]">Attached Links ({addedLinks.length}):</h5>
              <div className="flex flex-wrap gap-2">
                {addedLinks.map(link => (
                  <div key={link.id} className="bg-gray-100 border border-gray-200 text-[#3c3c3c] px-3.5 py-1.5 rounded-xl text-[13px] font-medium flex items-center gap-2">
                    ...
                    <a href={link.url} target="_blank" rel="noreferrer" className="underline truncate max-w-[240px] text-blue-600">{link.url}</a>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <h3 className="font-bold text-[16px] text-[#3c3c3c]">Add {activeLinkType} Link</h3>
              <button onClick={() => setActiveLinkType(null)} className="text-gray-400 hover:text-gray-600">
                <X className="size-5" />
              </button>
            </div>
            <div className="space-y-2">
              <label className="text-xs font-medium text-gray-600">Enter URL:</label>
              <input type="text" placeholder={`https://${activeLinkType.toLowerCase().replace(/\s+/g, '')}.com/...`}
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setActiveLinkType(null)}
                className="px-4 py-2 rounded-full text-xs font-medium bg-gray-100 hover:bg-gray-200 text-gray-700">Cancel</button>
              <button onClick={handleAddLink}
                className="px-5 py-2 rounded-full text-xs font-medium bg-[#06530b] hover:bg-emerald-900 text-white shadow-sm">Add Resource</button>
            </div>
          </div>
        </div>
      )}
