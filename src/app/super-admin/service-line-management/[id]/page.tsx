...
function SuperAdminServiceLineDetailsContent() {
    ...
    { key: 'lastLogin', header: 'Last Login', render: (val) => <span className="text-[13px] font-medium text-[#475569]">{String(val)}</span> },
    {
      ...
      render: (val) => (
        <div className="flex items-center gap-2">
          <Link href={`/super-admin/employees/${val}/edit?returnTo=/super-admin/service-line-management/${serviceLineId}?tab=Employee`} className="w-8 h-8 rounded-full flex items-center justify-center text-[#475569] hover:bg-gray-100 transition-colors">
            <Edit2 className="w-4 h-4" />
          </Link>
          <Link href={`/super-admin/employees/${val}?returnTo=/super-admin/service-line-management/${serviceLineId}?tab=Employee`} className="flex items-center gap-1.5 text-[#06530B] hover:text-green-800 font-bold text-xs transition-colors">
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-2.5 mb-3">
                      <div className="w-[36px] h-[36px] rounded-full bg-[#0F172A] text-white flex items-center justify-center font-bold text-sm shrink-0">
                        {team.name.charAt(0)}
                      </div>
                    ...
                    <div className="space-y-3.5 text-[12px]">
                      <div className="flex items-center justify-between">
                        <span className="text-[#0F172A]">Team Leader</span>
            ...
            <div className="w-full rounded-[16px] shadow-2xs bg-white">
              <DashboardTable
                ...
                columns={[
                  ...
                  { key: 'createDate', header: 'Group Date', render: (val) => <span className="text-[13px] font-medium text-[#475569]">{String(val)}</span> },
              ...
              {filteredEmployees.map((member) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2.5">
                      <Link
                        href={`/super-admin/employees/${member.id}/edit?returnTo=/super-admin/service-line-management/${serviceLineId}?tab=Employee`}
                        className="flex-1 h-[29px] bg-[rgba(125,125,125,0.13)] hover:bg-gray-300 text-[#333333] rounded-[8px] flex items-center justify-center gap-1 text-[14px] font-medium transition-colors cursor-pointer"
                      >
                        <Edit2 className="w-4 h-4" /> Edit
                      </Link>
                      <Link
                        href={`/super-admin/employees/${member.id}?returnTo=/super-admin/service-line-management/${serviceLineId}?tab=Employee`}
                        className="flex-1 h-[29px] bg-[#06530B] hover:bg-[#05290b] text-white rounded-[8px] flex items-center justify-center gap-1 text-[14px] font-semibold transition-colors cursor-pointer"
                      >
                        <Eye className="w-4 h-4" /> View
                      </Link>
              ...
              columns={[
                {
                  ...
                  header: 'Order ID',
                  render: (val) => (
                    <span className="font-bold text-[#06530b] font-['Roboto']">{String(val)}</span>
                  )
                },
                {
                  key: 'client',
                  header: 'Client name',
                  render: (val) => (
                    <span className="font-semibold text-[#1e293b] font-['Roboto']">{String(val)}</span>
                  )
                },
                {
                  key: 'profile',
                  header: 'Profile',
                  render: (val) => (
                    <span className="font-medium text-[#64748b] font-['Roboto']">{String(val)}</span>
                  )
                },
                {
                  key: 'team',
                ...
                },
                {
                  ...
                  header: 'Order Value',
                  render: (val) => (
                    <span className="font-bold text-[#0f172a] font-['Roboto']">{String(val)}</span>
                  )
                },
                {
                  key: 'initialSeconds',
                  ...
                  render: (_, item) => (
                    <CountdownTimer initialSeconds={item.initialSeconds} />
                  )
                },
                {
                  key: 'id',
                  ...
                  render: (val) => (
                    <Link
                      href={`/super-admin/service-line-management/${serviceLineId}/teams/1/projects/${val}`}
                      className="flex items-center gap-1.5 text-[#06530b] font-bold text-xs hover:underline cursor-pointer"
                    >
                      <Eye className="w-4 h-4" /> View
                    </Link>
                  )
          ...
          deletedViewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {mockDeletedTeams.map((team) => (
                <div
                  key={team.id}
                  className="bg-[#FAFAFA] border border-[#F2F2F2] rounded-[16px] p-[17px] flex flex-col justify-between hover:shadow-md transition-shadow opacity-90"
                >
                  <div>
                    {/* Header */}
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-[36px] h-[36px] rounded-full bg-[#94A3B8] text-white flex items-center justify-center font-bold text-sm shrink-0">
                          {team.name.charAt(0)}
                        </div>
                        <div className="flex flex-col">
                          <span className="font-semibold text-[#191C1E] text-[14px] leading-tight">
                          ...
                          <span className="text-[#464555] text-[12px] leading-tight">
                            Deleted: {team.deletedOn}
                          </span>
                        </div>
                      </div>
                      <span className="bg-red-50 text-red-600 text-[11px] font-bold px-2 py-0.5 rounded-full border border-red-200 shrink-0">
                        DELETED
                      </span>
                    </div>
...
export default function SuperAdminServiceLineDetailsDashboard() {
  return (
    <Suspense fallback={<div className="p-8 text-center">Loading...</div>}>
      <SuperAdminServiceLineDetailsContent />
    </Suspense>
  );
}
