...
export default function EditProjectWithIdPage() {
  const router = useRouter();
  const params = useParams();
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Extract dynamic fiver order id from URL parameter
  const routeOrderId = (params?.id as string) || 'FO2228CA90708';

  // Form states initialized with dynamic route id
  const [clientName, setClientName] = useState('alexjlouis');
  const [fiverrOrderId, setFiverrOrderId] = useState(routeOrderId);
  ...
  useEffect(() => {
    if (params?.id) {
      setFiverrOrderId(params.id as string);
    }
  }, [params]);

  // File Upload state
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([
    { id: '1', name: 'UI_Design_System_v2.pdf', size: '2.4 MB' }
  ]);

  // Resource Link state
  const [addedLinks, setAddedLinks] = useState<ResourceLink[]>([
    { id: '1', type: 'Figma', url: 'https://figma.com/file/tech-omega-handoff' }
  ]);
  ...
  const selectedCount = members.filter(m => m.selected).length + 1; // +1 for owner
    ...
    <div className="flex flex-col h-full max-h-full overflow-hidden space-y-6 select-none relative">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto space-y-6 pr-2">
        ...
        <div className="bg-white border border-[#e5e7eb] rounded-[24px] p-6 space-y-6 shadow-2xs">
          ...
          {/* Hidden File Input for Computer File Picker */}
          <input 
            type="file" 
            ref={fileInputRef} 
