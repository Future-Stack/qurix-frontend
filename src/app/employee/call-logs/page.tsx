...
export default function EmployeeCallLogsPage() {
  const [activeCallId, setActiveCallId] = useState<string | null>('1'); // Default to Theresa Webb
  ...
  const avatars = {
    ...
    mishu: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80', // falls back to similar
  ...
  const activeOnlineUsers = [
    { name: 'Annette', src: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80' },
    { name: 'Cody', src: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=150&q=80' },
    { name: 'Floyd', src: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80' },
    { name: 'Marvin', src: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?auto=format&fit=crop&w=150&q=80' }
  ];
  ...
  const renderCallIcon = (type: CallLogItem['type'], colorClass = '') => {
    switch (type) {
      case 'Outgoing':
        return <ArrowUpRight className={`size-4 text-emerald-600 ${colorClass}`} />;
      case 'Incoming':
      case 'Answered':
        return <ArrowDownLeft className={`size-4 text-emerald-600 ${colorClass}`} />;
      case 'Missed':
        return <PhoneMissed className={`size-4 text-red-500 ${colorClass}`} />;
      case 'Declined':
        return <PhoneOff className={`size-4 text-gray-500 ${colorClass}`} />;
      case 'Failed':
    ...
    <div className="flex flex-col h-[calc(100vh-72px)] md:h-[calc(100vh-54px)] lg:h-[calc(100vh-60px)] min-h-[600px] overflow-hidden -m-6 lg:-m-[30px]">
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className={`w-full lg:w-[469px] border-r border-[#dadada] flex flex-col h-full bg-white shrink-0 transition-transform duration-300 z-10 ${
          ...
          <div className="h-[129px] border-b border-[#dadada] shrink-0 flex items-center px-6 lg:px-[30px] gap-4">
          ...
          <div className="px-6 py-4 border-b border-slate-100 shrink-0">
            <div className="flex items-center gap-3.5 overflow-x-auto pb-1.5 scrollbar-none">
              {activeOnlineUsers.map((user) => (
                <div key={user.name} className="flex flex-col items-center gap-1 shrink-0 cursor-pointer select-none">
                  <div className="relative size-[56px]">
                    <div className="absolute inset-0 border-2 border-emerald-800/10 rounded-full" />
                    <div className="size-[56px] rounded-full overflow-hidden border border-white bg-gray-50 flex items-center justify-center shadow-sm">
          ...
          <div className="px-6 py-4 flex flex-col gap-4 shrink-0 bg-white">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 text-[#a19791]">
                <button className="p-1.5 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors" title="Filter calls">
              ...
              filteredLogs.map((log) => {
                ...
                return (
                  <div
                    ...
                    className={`p-[10px] rounded-[11px] flex items-center justify-between cursor-pointer select-none transition-all duration-150 ${
                      isActive 
                        ? 'bg-[#effff5] border border-emerald-50/50' 
                  ...
                  >
                    <div className="flex gap-[19px] items-center">
                      <div className="relative size-[56px] shrink-0">
                        ...
                        {log.isOnline && (
                          <span className="absolute bottom-0 right-0 size-[14px] bg-[#22c55e] border-2 border-white rounded-full animate-pulse" />
                        )}
                      </div>
        ...
        <div className={`flex-1 h-full bg-white transition-transform duration-300 flex flex-col ${
          ...
          {selectedLog ? (
            <>
              ...
              <div className="h-[129px] border-b border-[#dadada] shrink-0 flex items-center justify-between px-6 lg:px-[30px]">
                ...
                <div className="flex gap-[15px] items-center shrink-0">
                  <button className="bg-[#f7f5f4] size-[37px] flex items-center justify-center rounded-[18.5px] text-gray-700 hover:bg-slate-100 transition-colors cursor-pointer" title="Send message">
                    <MessageSquare className="size-5" />
                  </button>
                  <button className="bg-[#f7f5f4] size-[37px] flex items-center justify-center rounded-[18.5px] text-gray-700 hover:bg-slate-100 transition-colors cursor-pointer" title="Video call">
                    <Video className="size-5" />
                  </button>
                  <button className="bg-[#f7f5f4] size-[37px] flex items-center justify-center rounded-[18.5px] text-gray-700 hover:bg-slate-100 transition-colors cursor-pointer" title="Voice call">
                    <Phone className="size-5" />
                  </button>
                  <button className="p-1 text-gray-500 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors" title="More options">
              ...
              <div className="flex-1 overflow-y-auto p-6 lg:p-[30px] space-y-6 bg-slate-50/30">
                <div className="max-w-[753px] bg-white border border-[#dadada] rounded-[15px] p-6 shadow-sm">
                  <h5 className="font-['Roboto'] font-normal text-[#313131] text-[16px] mb-4">
                    Today
                  </h5>
                    ...
                    {selectedLog.history.map((hist) => (
                      <div key={hist.id} className="flex items-center justify-between">
                        <div className="flex gap-[9px] items-center">
                          <div className="bg-[#f7f5f4] size-10 rounded-full flex items-center justify-center shrink-0">
                            {renderCallIcon(hist.type, 'size-5')}
                          </div>
                          <div className="flex flex-col">
                            <span className="font-['Roboto'] text-gray-800 text-[14px] font-medium">
                              {hist.type} Call
                            </span>
                            <span className="font-['Roboto'] text-[#a19791] text-[12px] font-mono mt-0.5">
                              {hist.time}
                            </span>
                          </div>
                        </div>
