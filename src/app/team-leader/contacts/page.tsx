...
export default function TeamLeaderContactsPage() {
  // Default to chat id '2' (Cody Fisher) to match active state in Figma, allowing selection/deselection
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-72px)] md:h-[calc(100vh-54px)] lg:h-[calc(100vh-60px)] min-h-[600px] overflow-hidden -m-6 lg:-m-[30px]">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* Left Side Pane: Contact threads */}
        <div className={`w-full lg:w-[469px] border-r border-[#dadada] flex flex-col h-full bg-white shrink-0 transition-transform duration-300 z-10 ${
          activeChatId && !isDetailsOpen ? 'hidden lg:flex' : activeChatId && isDetailsOpen ? 'hidden lg:flex' : 'flex'
        }`}>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-2 bg-white scrollbar-thin">
            ...
            <div 
              ...
              className="bg-[#f7f5f4] hover:bg-orange-50/60 transition-colors duration-150 rounded-[11px] p-4 flex items-center justify-between cursor-pointer select-none mb-3 border border-transparent"
            >
            ...
            {filteredThreads.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-8 font-sans">No contacts match your search.</p>
            ) : (
              filteredThreads.map((thread) => (
                <ChatListItem
                  ...
                  onClick={() => {
                    ...
                    setIsDetailsOpen(false); // Close details when switching contacts
                  }}
                />
              ))
            )}
