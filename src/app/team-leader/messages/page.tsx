...
export default function TeamLeaderMessagesPage() {
  ...
  // Default to chat id '2' (tprice34) to display active group chatting state matching Figma, but allowing deselection for Welcome screen
  const [activeChatId, setActiveChatId] = useState<string | null>('2');
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [showArchived, setShowArchived] = useState(false);

  // Shared seconds counters for active ticking countdown
  ...
  const groupMembers = {
    default: [
      ...
      { name: 'UX Shakil', role: 'admin', avatar: avatars.shakil },
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-72px)] md:h-[calc(100vh-54px)] lg:h-[calc(100vh-60px)] min-h-[600px] overflow-hidden -m-6 lg:-m-[30px]">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* Right Side Pane: Group Details Sidebar */}
        {selectedThread && (
          <GroupDetailsSidebar
            ...
            members={selectedThread.members}
            onEdit={() => {
              const parts = selectedThread.name.split('||').map(p => p.trim());
