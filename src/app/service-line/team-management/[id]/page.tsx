...
export default function TeamManagementDetailsPage() {
  ...
  const projectColumns: Column<(typeof mockProjects)[0]>[] = [
    ...
    { key: 'seconds', header: 'Timeline', render: (_, item) => <CountdownTimer initialSeconds={item.seconds} /> },
    {
      ...
      header: 'Actions',
      render: (val) => (
        <Link href={`/service-line/team-management/${teamId}/projects/${val}`} className="flex items-center gap-1.5 text-[#06530B] font-bold text-xs hover:text-green-800 transition-colors">
  ...
  const memberColumns: Column<(typeof mockMembers)[0]>[] = [
    ...
    { key: 'lastLogin', header: 'Last login', render: (val) => <span className="text-[13px] font-medium text-[#475569]">{String(val)}</span> },
    {
      ...
      header: 'Action',
      render: (val) => (
        <Link href={`/service-line/employees/${val}`} className="flex items-center gap-1.5 text-[#06530B] hover:text-green-800 font-bold text-xs transition-colors">
          <Eye className="w-4 h-4" /> View
        </Link>
      )
    }
          ...
          viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {mockMembers.map((member) => (
                ...
                >
                  <div>
                    {/* Member Card Header (Figma 350:8635) */}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <Link
                        href={`/service-line/employees/${member.id}`}
